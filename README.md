...
- Failed checks can be **marked resolved** by whoever handled the corrective action, with a note of what was done
...
4. **GitHub Pages** — this repo deploys via Pages from `main` branch root. Custom domain is set via the `CNAME` file (currently `fbqc.lancelotbiz.com`) plus a DNS-only (not proxied) CNAME record at the registrar.
...
- **GARF** facility data hasn't been loaded yet (PARF, SRF, and KRF have been imported, with some manual cleanup needed after import due to path-naming mistakes — see the git history / chat log for the specific fixes made).
- **Toll-free SMS number** is being registered with Twilio for verification (Use Case: Account Notifications, internal staff alerts — covers QC checks, job-tracker-style alerts, and weather notices, not customer marketing).
